...
use crate::sounds::{SOUND1, SOUND2};
...
// Vettore con suono success, suono error e id drone
const SOUNDS_VEC: Vec<(String, String)> = vec![(SOUND1, SOUND2)];
...
impl Drone for MyDrone {
    fn new(options: DroneOptions) -> Self {
        Self {
            ...
            // Sound
            success_sound: SOUNDS_VEC[options.id].0,
            fail_sound: SOUNDS_VEC[options.id].1,
        }
    }
...
impl MyDrone {
    fn handle_packet(&mut self, packet: Packet) {
        match packet.pack_type {
            ...
            PacketType::Ack(_ack) =>
                //This scope is used to reproduce the sound
                {
                let rt = Runtime::new().unwrap();
                let url = &self.success_sound;
